    ...
    if len(downSampling) != 0:
        ...
        if saveProgress == True:
            try:
                ...
                iterRef[0]            = progressDict["optIter"]
        ...
        for s, sampStat in enumerate(downSampling):
            ...
            #main optimization
            err2Ave = lambda par : paraSquareErrorAve([par[:parXN], par[parXN:]], funcXY,\
                                                      [dataXforOpt, dataYforOpt],\
                                                      normXYRatio=normXYRatio,\
                                                      ratioHeadTail=ratioHeadTail,\
                                                      iterRef=iterRef, downSamp=[s, sampStat[:2]])
            paraFitResult = optimize.minimize(err2Ave, [*parXforOpt, *parYforOpt], method=optMethod,\
            ...
            if progressPlot == True:
                progressPlot_paraLeastSquare([parXforOpt, parYforOpt], funcXY,\
                                             [dataXforOpt, dataYforOpt], dataRangeXY, err2Ave,\
                                             iterRef=iterRef, downSamp=[s, sampStat[:2]])
            #save the progress
            if saveProgress == True:
                progressDict = {}
                progressDict["downSamplingN"] = s
                progressDict["optIter"]       = iterRef[0]
        ...
        paraFitResult = optimize.minimize(err2Ave, [*parXYinit[0], *parXYinit[1]],\
...
def paraSquareErrorAve(parXY, funcXY, dataXY, normXYRatio=[1.0, 1.0], paraRange=[0.0, 1.0],\
    ...
    err2Ave      = err2Sum/len(dataXY[0])
    err2HeadTail = err2HeadTail/len(dataXY[0])

    if len(iterRef) != 0:
        iterRef[0] += 1
        print("                                                        average norm square error =",\
              scientificStr_paraLeastSquare(err2Ave, 10))
        ...
        print("  norm head_tail square error =", scientificStr_paraLeastSquare(err2HeadTail, 10))
    return err2Ave + err2HeadTail
def progressPlot_paraLeastSquare(parXYFit, funcXY, dataXY, dataRangeXY, err2AveFunc,\
    ...
    plotTile += scientificStr_paraLeastSquare(err2AveFunc([*parXYFit[0], *parXYFit[1]]))
...
#                      [1e6, 1e6, None, None]]
    ...
    parXFit, parYFit = paraLeastSquare([initX, initY], [funcX, funcY], data, rangeXY,\
                                       ...
                                       iterRef=[0], progressPlot=saveBool, saveProgress=saveBool,\
